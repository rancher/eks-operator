...
func BuildUpstreamClusterState(name, managedTemplateID string, clusterState *eks.DescribeClusterOutput, nodeGroupStates []*eks.DescribeNodegroupOutput, ec2Service *ec2.EC2, includeManagedLaunchTemplate bool) (*eksv1.EKSClusterConfigSpec, string, error) {
	...
	if clusterState.Cluster.ResourcesVpcConfig.ClusterSecurityGroupId != nil {
		upstreamSpec.SecurityGroups = append(upstreamSpec.SecurityGroups, *clusterState.Cluster.ResourcesVpcConfig.ClusterSecurityGroupId)
	}
...
func (h *Handler) updateUpstreamClusterState(upstreamSpec *eksv1.EKSClusterConfigSpec, config *eksv1.EKSClusterConfig, clusterARN string, ngARNs map[string]string, eksService *eks.EKS, ec2Service *ec2.EC2, svc *cloudformation.CloudFormation) (*eksv1.EKSClusterConfig, error) {
	...
	for _, upstreamNg := range upstreamSpec.NodeGroups {
		...
		// there exists a launch template for this node group
		if upstreamNg.LaunchTemplate != nil {
			...
			// once the cluster is created, update the node group launch template with the default cluster security
			// group generated by EKS
			addDefaultClusterSecurityGroup := false
			if !reflect.DeepEqual(config.Status.SecurityGroups, upstreamSpec.SecurityGroups) {
				logrus.Infof("adding default security group to node group [%s] for cluster [%s]", aws.StringValue(upstreamNg.NodegroupName), config.Name)
				addDefaultClusterSecurityGroup = true
				config = config.DeepCopy()
			...
			if lt == nil && config.Status.ManagedLaunchTemplateID == aws.StringValue(upstreamNg.LaunchTemplate.ID) {
				...
				lt, err = newLaunchTemplateVersionIfNeeded(config, upstreamNg, ng, ec2Service, addDefaultClusterSecurityGroup)
				if err != nil {
					return config, err
				}
...
func (h *Handler) importCluster(config *eksv1.EKSClusterConfig, eksService *eks.EKS, ec2Service *ec2.EC2) (*eksv1.EKSClusterConfig, error) {
	...
	config.Status.SecurityGroups = append(config.Status.SecurityGroups, *clusterState.Cluster.ResourcesVpcConfig.ClusterSecurityGroupId)
	config.Status.Phase = eksConfigActivePhase
	return h.eksCC.UpdateStatus(config)
}
